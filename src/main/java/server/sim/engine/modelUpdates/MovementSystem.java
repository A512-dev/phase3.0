...
public final class MovementSystem implements MovementStrategy {
    ...
    final double Rslow   = server.sim.core.GameConfig.CSP_APPROACH_RADIUS;      // e.g., 120
    final double slowFac = server.sim.core.GameConfig.CSP_SLOW_FACTOR;          // e.g., 0.35
    final double margin  = server.sim.core.GameConfig.CSP_SOFTSTOP_MARGIN;      // e.g., 8
    final double rate    = server.sim.core.GameConfig.CSP_LERP_RATE;            // e.g., 12
...
//        final double DAMPING  = 0.995; // 1 → no damping
    ...
    */
        ...
        final double bulkCurveAcc  = server.sim.core.GameConfig.BULK_A_ACCEL_CURVE;     // e.g., 240 px/s^2
        final double bulkCurveVmax = server.sim.core.GameConfig.BULK_A_VMAX_CURVE;      // e.g., 160 px/s
        final double curveZoneR    = server.sim.core.GameConfig.CURVE_ZONE_RADIUS;      // e.g., 14 px around bends
        // ─── Bulk B (constant forward speed with periodic lateral deviation) ───
        final double bulkBSpeed   = server.sim.core.GameConfig.BULK_B_SPEED;
        final double bulkBAmpl    = server.sim.core.GameConfig.BULK_B_DEVIATION_AMPL;
        final double bulkBWave    = server.sim.core.GameConfig.BULK_B_DEVIATION_WAVELENGTH;
        final double bulkBTrack   = server.sim.core.GameConfig.BULK_B_LATERAL_TRACK_RATE; // convergence rate
...
//                    System.out.println("trtttttt");
            ...
            if (pkt instanceof server.sim.model.packet.confidentialPacket.types.ConfidentialLargePacket) {

                final double D = server.sim.core.GameConfig.CLP_TARGET_GAP;
                final double vmaxFwd = server.sim.core.GameConfig.CLP_MAX_SPEED_FWD;
                final double vmaxBack = server.sim.core.GameConfig.CLP_MAX_SPEED_BACK;
                final double rateCLP = server.sim.core.GameConfig.CLP_LERP_RATE;
...

            {
                ...
                boolean allowBackward = (pkt instanceof server.sim.model.packet.confidentialPacket.types.ConfidentialLargePacket);
...
//    }
    ...
    private double baseForwardCap(Packet pkt, double bulkFlatSpeed, double bulkBSpeed) {
        ...
        if (pkt instanceof server.sim.model.packet.bulkPacket.types.BulkPacketA)
            return bulkFlatSpeed;
        if (pkt instanceof server.sim.model.packet.bulkPacket.types.BulkPacketB)
            return bulkBSpeed;
